...
<div class="header-actions">
    <form method="GET" class="filter-form" action="{{ route('admin.belum_konfirmasi') }}">
        <label style="font-size:14px; color:#555; font-weight:bold;">Filter:</label>
        <input type="date" name="tgl1" value="{{ request('tgl1') }}" onchange="this.form.submit()">
        <span style="color:#777;">-</span>
        <input type="date" name="tgl2" value="{{ request('tgl2') }}" onchange="this.form.submit()">
        <a href="{{ route('admin.belum_konfirmasi') }}"><button type="button" style="background:#e74c3c;">Reset</button></a>
    </form>

    <div class="table-buttons">
    <button onclick="window.print()" class="btn-cetak">🖨 Cetak</button>
    <a href="{{ route('admin.export', 'belum') }}" class="btn-excel">📊 Export Excel</a>
    </div>
</div>
